...
session_start();
	...
	$step = $_GET["step"]+1;
	$sql_ap_plan1 = "SELECT MAX(ap_step) AS step From pcr_flow_approve WHERE ap_fm_id = '".$pcr_number."' AND ap_apr_id = 6 AND ap_ph_id = 1  ";
	...
	if($_POST["submit"] == 1){
		...
		if($query_a = mysqli_query($conn,$sql_a)){
			if($row_ap_plan1["step"] == $_GET["step"]){
				$sql_form = "UPDATE pcr_form SET checkk = 1 WHERE fm_pcr_number = '".$pcr_number."'"; 
				...
				$sql_select_dept = "SELECT * From pcr_flow_approve WHERE ap_step = '".$step."' AND ap_ph_id = 1 AND ap_fm_id = '".$pcr_number."'";
